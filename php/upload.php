<?php
session_start();
if (isset($_SESSION['success'])) {
    echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
            '.$_SESSION['success'].'
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
          </div>';
    unset($_SESSION['success']);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Upload Study Materials</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">

  <!-- Bootstrap -->
   <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <!-- Favicon -->
    <link rel="icon" type="image/png" href="../favicon.png">

</head>

<body>

<!-- Top Navbar -->

<div class="navbar-custom bg-primary text-white p-3 d-flex justify-content-between align-items-center" style="font-size: 20px;">
  <span class="fw-bold">Upload Study Materials</span>
  
  <a href="../php/admin-dashboard.php" class="btn btn-light btn-sm">
    ← Back to Dashboard
  </a>
</div>

<div class="container mt-5">
  <div class="row justify-content-center">
    <div class="col-md-8 col-sm-11">

      <div class="card upload-card p-4">
        <h4 class="text-center mb-4 fw-bold">📤 Upload New Material</h4>

        <form action="../php/add-materials.php" method="POST" enctype="multipart/form-data">

          <!-- Title -->
          <div class="mb-3">
            <label class="form-label">Title</label>
            <input type="text" name="title" class="form-control" placeholder="Enter material title" required>
          </div>

          <!-- Semester -->
          <div class="mb-3">
            <label class="form-label">Select Semester</label>
            <select name="semester" class="form-select" required>
              <option value="">Choose Semester</option>
              <option>Semester 1</option>
              <option>Semester 2</option>
              <option>Semester 3</option>
              <option>Semester 4</option>
              <option>Semester 5</option>
              <option>Semester 6</option>
              <option>Semester 7</option>
              <option>Semester 8</option>
            </select>
          </div>

          <!-- Subject -->
          <div class="mb-3">
            <label class="form-label">Subject Name</label>
            <input type="text" name="subject" class="form-control" placeholder="e.g. Data Structures and Algorithms" required>
          </div>

          <!--for material_type -->
          <div class="mb-3">
            <label class="form-label fw-bold">Material Category</label>
            <select name="material_type" class="form-select" required>
             <option value="Note">Lecture Notes</option>
             <option value="Question">Question Paper</option>
            </select>
          </div>

          <!-- File -->
          <div class="mb-4">
            <label class="form-label">Upload File</label>
            <input type="file" name="file" class="form-control" required>
          </div>

          <!--description-->
          <div class="mb-3">
            <label class="form-label fw-bold">Description</label>
            <textarea name="description" class="form-control" rows="2" placeholder="Briefly describe the content..."></textarea>
          </div>

          <!-- Button -->
          <button type="submit" name="upload" class="btn btn-primary w-100">
            Upload Material
          </button>

        </form>
      </div>

    </div>
  </div>
</div>

</body>
</html>
